...
const EXPLANATIONS: Record<string, RuleExplanation> = {
  ...
  CTX103: {
    ...
    severity: "warn",
    problem: "Agent instructions reference a file or directory that does not exist in the repository.",
    whyItMatters: "Agents following stale paths will produce incorrect results or waste tokens trying to navigate non-existent locations.",
    fix: "Update the path to point to an existing file or directory, or remove the reference.",
    safeAutofix: false,
  },
  CTX104: {
    code: "CTX104",
    title: "Unrecognized command reference",
    severity: "warn",
    problem: "Agent instructions reference a command that is not recognized as a known tool or available script.",
    whyItMatters: "Agents may attempt to run commands that do not exist, wasting tokens and producing errors.",
    fix: "Verify the command name, install the required tool, or update instructions to use available commands.",
    safeAutofix: false,
  },
  CTX105: {
    code: "CTX105",
